...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$bitcoin_qt_want_version" = "xauto" && test x$bitcoin_qt_force != xyes; then
    if test x$bitcoin_enable_qt != xno; then
      AC_MSG_WARN([$1; slc-qt frontend will not be built])
    fi
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build slc-qt GUI (default=auto, qt5 tried first)])],
    [
  ...
  if test x$bitcoin_qt_got_major_vers = x5; then
    ...
    if test x$bitcoin_cv_static_qt = xyes; then
      ...
      if test x$TARGET_OS = xwindows; then
        _BITCOIN_QT_CHECK_STATIC_PLUGINS([Q_IMPORT_PLUGIN(QWindowsIntegrationPlugin)],[-lqwindows -lfreetype -lbz2 -lz])
  ...
  if test x$bitcoin_qt_got_major_vers = x5; then
     ...
     if test x$use_pkgconfig = xyes; then
       ...
       if test x$TARGET_OS = xlinux; then
         ...
         QT_LIBS="-lQt5PlatformSupport $QT_LIBS -lopengl32"
     fi
     ])
  else
    if test x$qt_plugin_path != x; then
